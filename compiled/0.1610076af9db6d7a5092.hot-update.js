	  ...
	  _createClass(Map, [{
	    ...
	    value: function foundRestaurant(res) {
	      ...
	      pointQuery.on('layeradd', function (point) {
	        ...
	        var feature = marker.feature;
	        marker.setIcon(L.icon(feature.properties.icon));
	        var content = '<h2>' + feature.properties.title + '<\/h2>' + '<form>I would<br>' + ('<input type="radio" name="goBack' + pointQuery._leaflet_id + '" required> Definitely and absolutely<br>') + ('<input type="radio" name="goBack' + pointQuery._leaflet_id + '"> Never ever ever<br>') + 'go back<br>' + ('<input className="btn-default" type="button" id="fistBump' + pointQuery._leaflet_id + '" value="Thumbs!"></form>') + '<img src="' + feature.properties.image + '" alt="">' + ('<img id="wishImage" src="' + wishImage + '" alt="">') + (marker.feature.properties.icon.iconUrl == starFill ? '<img id="giftImage" src="' + giftImage + '"' : '');
